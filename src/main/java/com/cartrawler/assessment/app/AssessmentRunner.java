...
public class AssessmentRunner {
    ...
    public static void main(String[] args) {
        ...
        List<CarResult> list = CARS.stream().collect(Collectors.toList()); //Convert the Set into a List to beter manipulate

        List<CarResult> result = carutils.removeAnyDuplicate(list);  // Remove Duplicates

        display.formatDisplay(result);  //Display the List without duplicates

        List<CarResult> sortCarsbyCorporate = carutils.sortCarsbyCorporate(result); //Sort Cars by Corporate for example

        sortCarsbyCorporate.forEach(System.out::println); // Display sort cars by Corporate without table format

        Map<Object, List<CarResult>> map = carutils.sortSIPP(result); //Aggregate cars by different groups

        carutils.displayMap(map);
